...
/// Represents a single ingredient with its calorie contribution
struct IngredientBreakdown: Codable, Sendable {
    let name: String
    let calories: Double
...
struct FoodAnalysisResult: Codable, Sendable {
    ...
    let ingredients: [String]?  // Legacy: simple ingredient names
    let ingredientBreakdown: [IngredientBreakdown]?  // New: ingredients with calories

    enum CodingKeys: String, CodingKey {
        case name, servingSize, calories, protein, carbs, fat, fiber, sugar, salt, potassium, confidence, ingredients, ingredientBreakdown
    }

    // Regular memberwise initializer
    init(name: String, servingSize: String, calories: Double, protein: Double, carbs: Double, fat: Double, fiber: Double, sugar: Double, salt: Double = 0, potassium: Double = 0, confidence: String? = nil, ingredients: [String]? = nil, ingredientBreakdown: [IngredientBreakdown]? = nil) {
        self.name = name
    ...
    // Custom decoder to validate and cap values, and handle both ingredient formats
    nonisolated init(from decoder: Decoder) throws {
        ...
        // Try to decode ingredientBreakdown first (new format)
        ingredientBreakdown = try container.decodeIfPresent([IngredientBreakdown].self, forKey: .ingredientBreakdown)
        
        // Try to decode ingredients (legacy format: [String] or new format: [{name, calories}])
        if let stringIngredients = try? container.decodeIfPresent([String].self, forKey: .ingredients) {
            ingredients = stringIngredients
        } else if let objectIngredients = try? container.decodeIfPresent([IngredientBreakdown].self, forKey: .ingredients) {
            // AI returned ingredients as objects instead of strings - extract names for legacy field
            ingredients = objectIngredients.map { $0.name }
        } else {
            ingredients = nil
        }
        ...
        potassium = min(max(rawPotassium, 0), 5_000)
    }
    ...
    nonisolated func encode(to encoder: Encoder) throws {
        ...
        try container.encodeIfPresent(ingredientBreakdown, forKey: .ingredientBreakdown)
    }
    
    /// Converts ingredientBreakdown to a dictionary for storage in LoggedFood
    func ingredientCaloriesDictionary() -> [String: Double]? {
        guard let breakdown = ingredientBreakdown, !breakdown.isEmpty else { return nil }
        return Dictionary(uniqueKeysWithValues: breakdown.map { ($0.name, $0.calories) })
    }
}
