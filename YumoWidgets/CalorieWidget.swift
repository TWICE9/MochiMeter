...
struct CalorieProvider: TimelineProvider {
    ...
    func placeholder(in context: Context) -> CalorieEntry {
        CalorieEntry(date: .now, current: 1250, goal: 2100)
    }

    func getSnapshot(in context: Context, completion: @escaping (CalorieEntry) -> ()) {
        Task {
            let data = fetchCalorieData()
            completion(CalorieEntry(date: .now, current: data.current, goal: data.goal))
        }
    }
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<Entry>) -> ()) {
        Task {
            let data = fetchCalorieData()
            let entry = CalorieEntry(date: .now, current: data.current, goal: data.goal)
    ...
    private func fetchCalorieData() -> (current: Double, goal: Double) {
        ...
        case .systemMedium:
            HStack {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Text("\(Int(entry.current)) / \(Int(entry.goal)) kcal")
        ...
        case .accessoryRectangular:
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack {
                    ...
                    Spacer()
                    Text("\(Int(entry.current)) / \(Int(entry.goal))")
